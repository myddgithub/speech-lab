...
6. 图表上方的**工具行**（单行，图表独占整宽）：
   ...
   - 音节边界产生后，主图上会在每个边界处画**与波形同色的竖线**把波形分段，
     拖动边界时竖线实时跟随；
7. **💾 保存结果**：可**📦 一键保存全部**（ZIP：解码后原始+编辑后 WAV+多层 TextGrid+音高编辑点 PitchTier）或分别下载（TextGrid 旁即 **⬇️ PitchTier**）；TextGrid 为 Praat 直接可读（UTF-8 无 BOM，点层为标准 `TextTier`，区间层自动连续铺满）。
